...
private:
  ...
  std::vector<Frontier> findFrontiers(double rx, double ry) {
    ...
    // OPENNESS: fraction of FREE among the KNOWN cells (free + occupied) within openness_radius, ignoring
    // unknown. ~1.0 in an open room, low next to a wall. All-unknown surround -> 1.0 (keep first frontiers
    // attractive). Computed only for chosen/candidate cells (bounded work; hasClearance already prunes).
    const int opr = std::max(1, (int)std::lround(openness_radius_ / res));
    auto openness = [&](int cx, int cy) -> double {
      int nFreeO = 0, nOccO = 0;
      for (int dy = -opr; dy <= opr; ++dy)
        for (int dx = -opr; dx <= opr; ++dx) {
          int nx = cx + dx, ny = cy + dy;
          if (nx < 0 || ny < 0 || nx >= W || ny >= H) continue;
  ...
  double openness_radius_, openness_scale_, min_openness_, openness_band_;
