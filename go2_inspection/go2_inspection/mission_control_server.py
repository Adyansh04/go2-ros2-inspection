...
class MissionControl(Node):
    ...
    def _mission(self, zone_id="", inspect=True, return_home=True, goto_home=False, timeout=900):
        """Run the inspection_mission node as a subprocess; return (ok, manifest_dict)."""
    ...
    def _zone_result(self, man, zid):
        ...
        if os.path.exists(oj):
            try:
                base["objects_detail"] = json.load(open(oj))
            except Exception:
                pass
        return base
    ...
    def inspect_zone(self, req, res):
        ...
        if not ok:
            return self._ok(res, False, msg)
        try:
            okm, man = self._mission(zone_id=zid, inspect=True, return_home=False, timeout=900)
            zr = self._zone_result(man, zid)
            self._last = {"action": "inspect_zone", "zone": zid, "result": zr}
            return self._ok(res, okm, f"{zid}: {zr.get('n_objects', 0)} objects" if okm else f"inspect {zid} failed", zr)
        finally:
            self._release()
    ...
    def run_mission(self, req, res):
        ...
        if not ok:
            return self._ok(res, False, msg)
        try:
            okm, man = self._mission(zone_id=zones, inspect=True, return_home=True, timeout=3600)
            self._last = {"action": "run_mission", "result": man}
            msg = (f"mission: {man.get('total_objects', 0)} objects across {man.get('zones_with_objects', 0)} zones"
    ...
    def get_zone_image(self, req, res):
        ...
        oj = os.path.join(d, "objects.json")
        objects = None
        if os.path.exists(oj):
            try:
                objects = json.load(open(oj))
            except Exception:
                pass
        payload = {"zone": zid,
    ...
    def get_zone_gauges(self, req, res):
        """Detected objects for a zone (service name kept stable for the MCP layer; returns objects.json
        + the human-readable report.csv)."""
        zid = req.zone_id.strip()
        d = os.path.join(GAUGES_ROOT, zid)
        out = {"zone": zid}
        oj, csv = os.path.join(d, "objects.json"), os.path.join(d, "report.csv")
        if os.path.exists(oj):
            try:
                out["objects"] = json.load(open(oj))
            except Exception:
                pass
        if os.path.exists(csv):
            out["report_csv"] = open(csv).read()
        ok = "objects" in out
        n = out.get("objects", {}).get("n_objects", 0)
        return self._ok(res, ok, f"{n} objects" if ok else f"no objects.json for '{zid}'", out)
    ...
    def get_report(self, req, res):
        ...
        man = json.load(open(MANIFEST))
        return self._ok(res, True,
                        f"{man.get('total_objects', 0)} objects across {man.get('zones_with_objects', 0)} zones", man)
