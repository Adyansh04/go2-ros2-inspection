...
class MissionControl(Node):
    ...
    def _mission_cmd(self, zone_id="", inspect=True, return_home=True, goto_home=False):
        return [
            "ros2",
            "run",
            "go2_inspection",
    ...
    def _start_mission(
        ...
        try:
            proc = subprocess.Popen(
                self._mission_cmd(zone_id, inspect, return_home, goto_home),
                env=_env(),
    ...
    def inspect_zone(self, req, res):
        ...
        if err:
            return self._ok(res, False, err)
        return self._start_mission(
            res, f"inspect_zone:{zid}", zone_id=zid, inspect=True, return_home=False, timeout=900
        )
    ...
    def run_mission(self, req, res):
        ...
        if zones:  # a subset -> validate each id so an all-unknown list
            ...
            zones = ",".join(req_ids)
        return self._start_mission(
            res, "run_mission", zone_id=zones, inspect=True, return_home=True, timeout=3600
        )
