...
def _parse_map_yaml(path):
    ...
    origin = json.loads(d["origin"]) if d["origin"].startswith("[") else [float(x) for x in d["origin"].split()]
    img_name = d["image"]
...
def _draw_objects(img, objects, res, ox, oy, H, label=True):
    for o in objects:
        ...
        if label:
            cv2.putText(img, o["class"], (px + 7, py + 3), cv2.FONT_HERSHEY_SIMPLEX, 0.42, col, 1,
                        cv2.LINE_AA)
...
def plot_zone_map(zone_id, polygon, viewpoints, objects, out_path, map_yaml=None):
    ...
    cv2.putText(img, f"{zone_id}: {len(objects)} objects", (8, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
...
def plot_facility_map(zones_objects, out_path, zone_polys=None, map_yaml=None):
    ...
    for zid, objs in zones_objects.items():
        ...
        total += len(objs)
    cv2.putText(img, f"facility: {total} objects across {len(zones_objects)} zones", (8, 22),
...
def write_zone_report(zone_id, label, objects, out_dir):
    ...
    with open(md, "w") as f:
        ...
        for i, o in enumerate(objects):
            ...
            f.write(f"| {i} | {o['class']} | {o['confidence']:.2f} | {ws} | "
                    f"{o.get('n_observations', 1)} | {o.get('localized', False)} | {o.get('crop', '')} |\n")
    with open(csvp, "w", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(["id", "zone", "class", "confidence", "x", "y", "z", "n_observations", "localized", "crop"])
        for o in objects:
            w = o.get("world") or [None, None, None]
            wr.writerow([o.get("id"), zone_id, o["class"], round(o["confidence"], 3),
                         w[0], w[1], w[2], o.get("n_observations", 1), o.get("localized", False),
                         o.get("crop", "")])
    return md, csvp
