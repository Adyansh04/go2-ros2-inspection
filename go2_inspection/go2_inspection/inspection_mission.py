...
NO hard-coded poses -> works in any world.
...
Detection + report only (no gauge reading). Everything per-room is DISCOVERED, not configured.
...
def inspect_zone(zone_id, zones_file, map_yaml):
    ...
    cmd = ["ros2", "run", "go2_inspection", "zone_inspector", "--ros-args",
           "-p", "use_sim_time:=true",
           "-p", f"zone_id:={zone_id}", "-p", f"zones_file:={zones_file}", "-p", f"map_yaml:={map_yaml}"]
...
def _facility_rollup(zones_file, map_yaml, results):
    ...
    zones_objects = {}; zone_polys = {}
    for r in results:
        zid = r["zone"]
        oj = os.path.join(GAUGES_ROOT, zid, "objects.json")
        if os.path.exists(oj):
            objs = json.load(open(oj)).get("objects", [])
            zones_objects[zid] = objs; zone_polys[zid] = zones.get(zid, {}).get("polygon", [])
    total = sum(len(v) for v in zones_objects.values())
    rooms_ok = sum(1 for v in zones_objects.values() if v)
    json.dump({"rooms": [{"zone": r["zone"], "label": zones.get(r["zone"], {}).get("label", ""),
                          "nav": r["nav"], "n_objects": len(zones_objects.get(r["zone"], []))}
                         for r in results],
               "total_objects": total, "zones_with_objects": rooms_ok},
              open(MANIFEST, "w"), indent=2)
    try:
        if zones_objects:
            report_utils.plot_facility_map(zones_objects, os.path.join(GAUGES_ROOT, "facility_map.png"),
                                           zone_polys, map_yaml)
        with open(os.path.join(GAUGES_ROOT, "facility_report.md"), "w") as f:
            f.write(f"# Facility inspection report\n\n**{total} objects across {rooms_ok} zone(s).**\n\n")
            for zid, objs in zones_objects.items():
                lbl = zones.get(zid, {}).get("label", "")
                f.write(f"- **{zid}** ({lbl}): {len(objs)} object(s) -- "
...
def main(args=None):
    ...
    results = []
    for z in cands:
        zid = z["id"]; gx, gy = z.get("nav_point") or z["center"]      # prefer the free-space nav point
        if not m.goto(gx, gy, label=f"zone {zid}"):
            results.append({"zone": zid, "nav": False, "n_objects": 0}); continue
        if not m.inspect:
            results.append({"zone": zid, "nav": True, "n_objects": 0}); continue
    ...
    if m.inspect:
        ...
        m.get_logger().info(f"=== MISSION COMPLETE -> {MANIFEST} : {total} objects across {rooms_ok} zones ===")
    m.destroy_node()
    if rclpy.ok():
        rclpy.shutdown()
