...
Pipeline:
  1. OBSTACLE-ISLAND FILL (the key robustness fix): free-standing props (drums, pallets, racks, boxes,
...
def _quadrant_label(cx, cy, ox, oy, W, H, res):
    ...
    fx = (cx - ox) / (W * res); fy = (cy - oy) / (H * res)
    ns = "S" if fy < 1 / 3 else ("N" if fy > 2 / 3 else "")
    we = "W" if fx < 1 / 3 else ("E" if fx > 2 / 3 else "")
    return (ns + we) or "center"


def segment_grid(grid, res, ox, oy, core_dist_m=1.6, min_area_m2=6.0,
                 island_max_m2=2.5, wall_span_m=3.0):
    ...
    cv2.watershed(cv2.cvtColor(free_clean * 255, cv2.COLOR_GRAY2BGR), markers)  # flood free space from cores
    zones = []
    for lab in range(2, int(markers.max()) + 1):
        ...
        zones.append({
            "center": [cx_w, cy_w],
            "nav_point": [round(float(ox + np_idx[1] * res), 3), round(float(oy + np_idx[0] * res), 3)],
...
def _viz(grid, markers, zones, res, ox, oy, path):
    ...
    for z in zones:
        ...
        cv2.putText(out, z["id"].replace("zone_", "Z") + ":" + z.get("label", ""), (cx - 14, cy - 8),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        nx = int((z["nav_point"][0] - ox) / res); ny = H - 1 - int((z["nav_point"][1] - oy) / res)
...
def main_standalone(npz_path):
    ...
    for z in zones:
        print(f"  {z['id']} [{z['label']}]: center={z['center']} nav_point={z['nav_point']} "
    ...
    class ZoneSegmenter(Node):
        ...
        def publish_markers(self, zones):
            ...
            for i, z in enumerate(zones):
                ...
                tx.pose.position.z = 0.3; tx.pose.orientation.w = 1.0
