...
def main():
    text = open(sdf).read()
    walls = re.findall(
        r'<collision name="w\d+_c"><pose>([^<]+)</pose><geometry><box><size>([^<]+)</size>', text
    )
    ...
    viz = np.full((H, W, 3), 128, np.uint8)
    viz[grid == 0] = 255
    viz[grid == 100] = 0
    cv2.imwrite(os.path.join(worlds, "facility_map_viz.png"), cv2.flip(viz, 0))
    print(
        f"saved {out}: {W}x{H} @ {RES}m  walls={len(walls)}  free={int((grid==0).sum())}  unknown={int((grid==-1).sum())}"
    )
