#!/usr/bin/env python3
"""Create facility_inspection.sdf = facility.sdf plus analog gauges distributed across multiple rooms.

A realistic multi-room inspection scenario: instead of all gauges on one wall (facility_gauges.sdf),
...
Additive and non-destructive: facility.sdf, facility_gauges.sdf, gauge_tex/ and the original generators are
left untouched. This writes a separate world (facility_inspection.sdf), a separate texture set
(inspection_tex/) behind its own no-space symlink, and inspection_groundtruth.json. Reuses draw_gauge.

  python3 gen_inspection_world.py      # re-run per machine to refresh the textures and the symlink
...
doorways, so autonomous navigation and the strafe sweep have room to work. Each gauge panel faces
...
LINK = os.path.expanduser(
    "~/.go2_inspection_tex"
)  # no-space symlink (gz can't resolve the space path)
...
def main():
    ...
    print(
        f"  {len(LAYOUT)} assets across {len(rooms)} rooms, {sum(g['anomaly'] for g in LAYOUT)} anomalies:"
    )
