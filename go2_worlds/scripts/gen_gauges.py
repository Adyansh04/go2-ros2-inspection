...
def draw_gauge(spec, path):
    ...
    for i in range(nmaj + 1):
        ...
        if i < nmaj:  # 4 minor ticks between majors
            for j in range(1, 5):
                ...
                ax.plot(
                    [0.85 * cm, 0.92 * cm], [0.85 * sm, 0.92 * sm], color="black", lw=1, zorder=3
                )
    ...
    # the textured panel render see-through (gauges appear as blank gray). Emitting plain RGB avoids it.
